...
authority:
  - docs/mvp/PRD-PHASE1-MVP.md (v2.1)
  - docs/decisions/adr/ADR-INDEX.md (ADR-001–035)
  - docs/usecases/USE-CASE-CATALOG.md
...
### Amendment 2026-09-06 (v1.2)

The repository is standalone (`tennisapp`). ADR-036 adds store presence through shells — a Trusted Web Activity on Google Play and a Capacitor shell on the App Store — sequenced after the pilot cluster is live on the web and before the city gate. The stories above are unchanged; the shells distribute the same web application and honour `minSupportedClient` from the status capability. Tickets 23–26 carry that work; `docs/roadmap/NATIVE-AND-STORES.md` is the plan.
