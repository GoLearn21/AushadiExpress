...
- [ ] A push to the branch deploys a Vercel preview; the preview's status capability responds — **blocked: no Vercel credentials in this environment; run `scripts/provision.sh` on a machine logged into Vercel, or add `VERCEL_TOKEN` to the environment**
...
- The Kotlin suite declares `fixtures/` as a Gradle task input; without that, a fixture change served a cached green locally.
