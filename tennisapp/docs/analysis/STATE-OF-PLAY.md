...
**As of:** 2026-09-06 — the project now lives in its own repository (`tennisapp`, ADR-036); see ADR-030–036 for the decisions taken since 2026-08-27; the spec is at `.scratch/rally-phase1/spec.md` (137 stories, two seams); tickets 01–26 in `.scratch/rally-phase1/issues/`
...
| Decisions | `decisions/adr/ADR-INDEX.md` | ADR-001–036 + one amendment |
...
| Code (reference) | `kotlin-reference/shared/` | 94 tests, 0 failures, JVM target only |
| Code (product) | `api/`, `domain/`, `web/`, `fixtures/` | 43 tests, 0 failures; status capability at Seam 1, canon at Seam 2, token contrast gate |
| Store apps | `docs/roadmap/NATIVE-AND-STORES.md`, tickets 23–26 | Planned (ADR-036); founder account tasks start now |
| Conversation | `docs/CONVERSATION-RECORD.md` | Full founder/Claude record, 2026-08-23 → 09-06 |
...
12. **Both platforms ship.** Shipping one halves liquidity in a market whose entire thesis is
    liquidity.
13. **Store presence comes through shells before the native decision** (ADR-036, 2026-09-06):
    a Trusted Web Activity on Google Play, a Capacitor shell with push and share on the App
    Store. Native UI is still the ADR-031 spike at the city gate. Recorded for veto.

---

## 3. What is built, and what it is worth

`kotlin-reference/shared/` — Kotlin 2.4.10, JVM target only, 92 tests.
...
| 6 | ~~Run `/to-tickets`~~ Done: tickets 01–26 exist | — |
| 7 | **Veto or confirm ADR-036** (store shells before native; `Rally Tennis` as store title pending trademark search). Silence is confirmation. | Founder |
| 8 | Run `scripts/provision.sh` on the Mac to create the Supabase and Vercel projects — closes ticket 01 | Founder |
| 9 | Domain, trademark search, Apple Developer and Play Console accounts — `HANDOFF.md` §7 | Founder |
