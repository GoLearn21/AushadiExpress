...
| 036 | Standalone repository; store presence through shells before the native decision | Accepted — **amends 031, recorded for veto** | 95% |
...
**Context.** `kotlin-reference/shared/` is 92 tests of correct, portable domain logic: the versioned
...
## ADR-036 — Standalone repository; store presence through shells before the native decision {#adr-036}

**Context.** On 2026-09-06 the founder directed that the product move out of the pharmaceutical
marketplace repository entirely — its own folder on the Mac, its own GitHub repository, its own
Supabase and Vercel projects — and stated a requirement that ADR-031 had left to the city gate:
*"we need android app and ios app as well on their playstore and apple store to easily be
identifiable or discoverable and usable by anyone."*

ADR-031 chose mobile web for Phase 1 on speed and on the cost of being wrong in a no-OTA binary,
and left native for a one-week CMP-vs-SwiftUI spike at the city gate. It did not separate two
things that are separate: **native UI** (the thing the spike decides) and **store presence**
(being findable in the App Store and Google Play, installed with one tap, and reached by push).
A player who searches "tennis" in a store never finds a web app, however good it is.

**Decision.**

1. **The repository is `tennisapp`, standalone.** The TypeScript workspaces sit at the root; the
   Kotlin reference domain is `kotlin-reference/`; the corpus is `docs/`; the spec and tickets
   stay in `.scratch/rally-phase1/`. Nothing references the former host repository. The Supabase
   project and the Vercel project are new, created by the founder on a logged-in machine through
   `scripts/provision.sh`; no credential from any other product is reused.

2. **Store presence is earned through shells around the web app, not deferred to the native
   decision.** Both stores are entered with the same served web application:
   - **Google Play through a Trusted Web Activity** (Bubblewrap / PWABuilder). The shell is a
     signed Android package whose content is the live site, verified through Digital Asset Links
     served from the web domain. It carries no application code, so there is no OTA freeze —
     *server ranks, client renders* (PRD §5) holds exactly as on the web.
   - **The App Store through a Capacitor shell** with bundled web assets and at least two
     native capabilities the web cannot offer on iOS: **APNs push** and the **native share
     sheet** for invites. Apple's minimum-functionality guideline rejects a bare repackaged
     website; a shell that carries push, share and haptics is the form that passes review in
     practice. The bundled assets make every App Store release a review cycle, so the shell
     must respect the `min_supported_client` already served by `/status`, and the ranking,
     copy and policy stay server-supplied.
   The shells are tickets 23–26. Ticket 23 (identity and accounts) starts **now**, because
   developer-account verification, a D-U-N-S number and a trademark search take calendar weeks
   and cost nothing in engineering time. Tickets 24–26 are sequenced immediately after the pilot
   cluster is live on the web — before the city gate, not at it. The founder can pull them
   earlier with one word; the cost is about two weeks and one store-review cycle per release.

3. **ADR-031's spike is unchanged.** Native *UI* (Compose Multiplatform against SwiftUI) is still
   decided at the city gate, on the availability-grid accessibility spike. The shells are what
   the native UI replaces when it is earned; until then they are the store presence.

4. **The name is a store decision, not a code decision.** "Rally" stays as the product's working
   name and the code's package (`app.rally`). The store title, subtitle and bundle identifier are
   fixed in ticket 23 after a USPTO clearance search and a domain purchase, because a store title
   that collides with existing "Rally" apps is precisely the discoverability failure the founder
   is asking to avoid.

**Status.** Accepted 2026-09-06 under the delegated governor role. **Amends ADR-031** (store
presence no longer waits for the native decision). Recorded for veto: silence confirms.

**Consequences.**
- One link works everywhere: Universal Links (`apple-app-site-association`) and Android App Links
  (`assetlinks.json`) are served by the web deployment, so an invite (ADR-015) opens the store
  app if installed and the web app if not. This is the invite loop's cheapest conversion.
- `notification_delivery(channel)` (ADR-035) gains its second and third channels: APNs and FCM
  beside web push, with the same delivery rows and the same observability.
- The store shells double the release surface. Fastlane automates both submissions from the
  Mac (ticket 26); screenshots, privacy forms and the age rating are assets in the repository,
  not memories.
- iOS web push remains limited to home-screen installs; the App Store shell is the only way most
  iPhone players receive an offer notification. This is the concrete reason the shell is worth
  a review cycle.

**Alternatives rejected.**
- *Wait for the city gate* (ADR-031 as written). Rejected: it conflates native UI with store
  presence, and leaves iPhone players without reliable push through the entire pilot.
- *Go native now* (ADR-025 as originally accepted). Rejected on the unchanged arithmetic of
  ADR-031: the ranking and copy would freeze in a binary before the pilot has taught anything.
- *A single Capacitor shell for both stores.* Rejected for Android: a TWA has no bundled code,
  so Android never needs a store release for a product change; Capacitor on Android would
  forfeit that for no gain.
- *Keep the product inside the host repository with a separate CI path.* Rejected: two
  unrelated products in one issue tracker, one CI, one set of secrets and one commit history is
  the wrong shape for a project that is about to have its own cloud accounts.

**Confidence: 95%.** What would raise it: an App Store approval of the Capacitor shell on the
first submission. What would reverse it: a rejection under guideline 4.2 that survives a resubmit
with push and share present — in which case ticket 25 becomes the SwiftUI branch of the ADR-031
spike, pulled forward.
