...
export default function WholesalerInvoiceScannerScreen() {
  ...
  // Process invoice mutation for wholesaler
  const processInvoice = useMutation({
    mutationFn: async (imageData: string) => {
      const response = await fetch('/api/wholesaler/invoices/process', {
        method: 'POST',
    ...
    onSuccess: (data: any) => {
      toast({
        ...
        description: `Added ${data.itemCount || 0} products to your catalog`,
      });
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-white font-semibold">
                Stock Invoice Scanner
              </h1>
              <p className="text-white/70 text-xs">
                Scan purchase invoices to update inventory
              </p>
            </div>
          </div>

          {showWebFallback && !capturedImage && (
            <Button
              size="sm"
              variant="secondary"
        ...
        {capturedImage ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative">
                ...
                <div className="w-80 h-56 border-2 border-white rounded-lg relative">
                  ...
                  {/* Document Icon */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="material-icons text-white text-4xl opacity-30">receipt_long</span>
                  </div>
                </div>

                <p className="text-white text-center mt-4 text-sm">
                  Position purchase invoice within frame
                </p>
                <p className="text-white/80 text-center mt-1 text-xs">
                  AI will extract product details & quantities
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center text-white p-8">
              {showWebFallback ? (
                <>
                  <span className="material-icons text-6xl mb-4 opacity-60">add_photo_alternate</span>
                  <h2 className="text-xl font-semibold mb-2">Upload Invoice Photo</h2>
                  <p className="text-white/80 mb-4">Camera not available. Upload a photo of your purchase invoice instead.</p>
                  <Button onClick={handleWebFallback} className="bg-green-600 hover:bg-green-700">
